  ...
  module Filestorage
    class Issue
      ...
      def self.from_json(json_object:)
        ...
        id = struct.id
        status = Filestorage::ISSUE_STATUS_ENUM.key(parsed_json["status"]) || parsed_json["status"]
      ...
      def to_json(*_args)
        {
          "id": @id,
          "status": Filestorage::ISSUE_STATUS_ENUM[@status] || @status,
          "error_description": @error_description,
          "end_user": @end_user,
          "first_incident_time": @first_incident_time,
          "last_incident_time": @last_incident_time,
      ...
      def self.validate_raw(obj:)
        ...
        obj.status&.is_a?(Filestorage::ISSUE_STATUS_ENUM) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
