  ...
  module Accounting
    class AuditLogEvent
      ...
      def self.from_json(json_object:)
        ...
        role = Accounting::ROLE_ENUM.key(parsed_json["role"]) || parsed_json["role"]
        ip_address = struct.ip_address
        event_type = Accounting::EVENT_TYPE_ENUM.key(parsed_json["event_type"]) || parsed_json["event_type"]
      ...
      def to_json(*_args)
        {
          ...
          "user_name": @user_name,
          "user_email": @user_email,
          "role": Accounting::ROLE_ENUM[@role] || @role,
          "ip_address": @ip_address,
          "event_type": Accounting::EVENT_TYPE_ENUM[@event_type] || @event_type,
          "event_description": @event_description,
          "created_at": @created_at
        }.to_json
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.role.is_a?(Accounting::ROLE_ENUM) != false || raise("Passed value for field obj.role is not the expected type, validation failed.")
        obj.ip_address.is_a?(String) != false || raise("Passed value for field obj.ip_address is not the expected type, validation failed.")
        obj.event_type.is_a?(Accounting::EVENT_TYPE_ENUM) != false || raise("Passed value for field obj.event_type is not the expected type, validation failed.")
