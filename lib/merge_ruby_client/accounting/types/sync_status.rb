  ...
  module Accounting
    ...
    class SyncStatus
      ...
      def self.from_json(json_object:)
        ...
        status = Accounting::SYNC_STATUS_STATUS_ENUM.key(parsed_json["status"]) || parsed_json["status"]
        is_initial_sync = struct.is_initial_sync
        selective_sync_configurations_usage = Accounting::SELECTIVE_SYNC_CONFIGURATIONS_USAGE_ENUM.key(parsed_json["selective_sync_configurations_usage"]) || parsed_json["selective_sync_configurations_usage"]
      ...
      def to_json(*_args)
        {
          ...
          "last_sync_start": @last_sync_start,
          "next_sync_start": @next_sync_start,
          "status": Accounting::SYNC_STATUS_STATUS_ENUM[@status] || @status,
          "is_initial_sync": @is_initial_sync,
          "selective_sync_configurations_usage": Accounting::SELECTIVE_SYNC_CONFIGURATIONS_USAGE_ENUM[@selective_sync_configurations_usage] || @selective_sync_configurations_usage
        }.to_json
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.status.is_a?(Accounting::SYNC_STATUS_STATUS_ENUM) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
        ...
        obj.selective_sync_configurations_usage&.is_a?(Accounting::SELECTIVE_SYNC_CONFIGURATIONS_USAGE_ENUM) != false || raise("Passed value for field obj.selective_sync_configurations_usage is not the expected type, validation failed.")
      end
    end
  end
end
