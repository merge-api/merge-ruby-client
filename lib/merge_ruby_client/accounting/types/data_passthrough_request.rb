...
module Merge
  module Accounting
    ...
    class DataPassthroughRequest
      # @return [Merge::Accounting::MethodEnum]
      attr_reader :method_
      ...
      attr_reader :data
      # @return [Array<Merge::Accounting::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the
      ...
      attr_reader :headers
      # @return [Merge::Accounting::RequestFormatEnum]
      ...
      # @param method_ [Merge::Accounting::MethodEnum]
      ...
      #  matching the data's format
      # @param multipart_form_data [Array<Merge::Accounting::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the
      #  `data` param if `request_format` is set to `MULTIPART`.
      # @param headers [Hash{String => Object}] The headers to use for the request (Merge will handle the account's
      #  authorization headers). `Content-Type` header is required for passthrough.
      #  Choose content type corresponding to expected format of receiving server.
      # @param request_format [Merge::Accounting::RequestFormatEnum]
      ...
      # @return [Merge::Accounting::DataPassthroughRequest]
      ...
      def self.from_json(json_object:)
        ...
        multipart_form_data = parsed_json["multipart_form_data"]&.map do |item|
          item = item.to_json
          Merge::Accounting::MultipartFormFieldRequest.from_json(json_object: item)
        end
      ...
      def self.validate_raw(obj:)
        obj.method_.is_a?(Merge::Accounting::MethodEnum) != false || raise("Passed value for field obj.method_ is not the expected type, validation failed.")
        ...
        obj.request_format&.is_a?(Merge::Accounting::RequestFormatEnum) != false || raise("Passed value for field obj.request_format is not the expected type, validation failed.")
