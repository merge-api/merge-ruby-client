...
module Merge
  module Accounting
    class SyncStatusClient
      ...
      # @return [Accounting::SyncStatusClient]
      ...
      # @return [Accounting::PaginatedSyncStatusList]
      def list(cursor: nil, page_size: nil, request_options: nil)
        response = @request_client.conn.get("/api/accounting/v1/sync-status") do |req|
        ...
        Accounting::PaginatedSyncStatusList.from_json(json_object: response.body)
      end
    end
    ...
    class AsyncSyncStatusClient
      ...
      # @return [Accounting::AsyncSyncStatusClient]
      def initialize(request_client:)
