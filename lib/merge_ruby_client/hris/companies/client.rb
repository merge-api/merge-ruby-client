...
module Merge
  module Hris
    class CompaniesClient
      ...
      # @return [Hris::PaginatedCompanyList]
      def list(created_after: nil, created_before: nil, cursor: nil, include_deleted_data: nil,
        ...
        response = @request_client.conn.get("/api/hris/v1/companies") do |req|
        ...
        Hris::PaginatedCompanyList.from_json(json_object: response.body)
      end
      ...
      def retrieve(id:, include_remote_data: nil, request_options: nil)
        response = @request_client.conn.get("/api/hris/v1/companies/#{id}") do |req|
          req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
          req.headers["Authorization"] = request_options.api_key unless request_options&.api_key.nil?
          req.headers["X-Account-Token"] = request_options.account_token unless request_options&.account_token.nil?
          req.headers = { **req.headers, **(request_options&.additional_headers || {}) }.compact
        ...
        Hris::Company.from_json(json_object: response.body)
      end
    end
    ...
    class AsyncCompaniesClient
      ...
      # @return [Hris::AsyncCompaniesClient]
      def initialize(request_client:)
