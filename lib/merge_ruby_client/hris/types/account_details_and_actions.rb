...
module Merge
  module Hris
    ...
    class AccountDetailsAndActions
      ...
      # @param integration [Hris::AccountDetailsAndActionsIntegration]
      # @param account_type [String]
      # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
      # @return [Hris::AccountDetailsAndActions]
      def initialize(id:, status:, end_user_organization_name:, end_user_email_address:, webhook_listener_url:,
        ...
        # @type [Hris::AccountDetailsAndActionsIntegration]
      ...
      def self.from_json(json_object:)
        ...
        id = struct.id
        category = Hris::CATEGORY_ENUM.key(parsed_json["category"]) || parsed_json["category"]
        status = Hris::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM.key(parsed_json["status"]) || parsed_json["status"]
        ...
        if parsed_json["integration"].nil?
          ...
          integration = Hris::AccountDetailsAndActionsIntegration.from_json(json_object: integration)
        end
      ...
      def to_json(*_args)
        {
          "id": @id,
          "category": Hris::CATEGORY_ENUM[@category] || @category,
          "status": Hris::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM[@status] || @status,
          "status_detail": @status_detail,
          "end_user_origin_id": @end_user_origin_id,
          "end_user_organization_name": @end_user_organization_name,
          "end_user_email_address": @end_user_email_address,
      ...
      def self.validate_raw(obj:)
        ...
        obj.category&.is_a?(Hris::CATEGORY_ENUM) != false || raise("Passed value for field obj.category is not the expected type, validation failed.")
        obj.status.is_a?(Hris::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
        ...
        obj.integration.nil? || Hris::AccountDetailsAndActionsIntegration.validate_raw(obj: obj.integration)
