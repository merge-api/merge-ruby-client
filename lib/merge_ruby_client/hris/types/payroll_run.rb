...
module Merge
  module Hris
    ...
    class PayrollRun
      ...
      # @param run_state [RUN_STATE_ENUM] The state of the payroll run
      #   - `PAID` - PAID
      #   - `DRAFT` - DRAFT
      #   - `APPROVED` - APPROVED
      #   - `FAILED` - FAILED
      #   - `CLOSED` - CLOSED
      # @param run_type [RUN_TYPE_ENUM] The type of the payroll run
      #   - `REGULAR` - REGULAR
      #   - `OFF_CYCLE` - OFF_CYCLE
      ...
      def initialize(id: nil, remote_id: nil, run_state: nil, run_type: nil, start_date: nil, end_date: nil,
        ...
        @remote_id = remote_id
        # @type [RUN_STATE_ENUM] The state of the payroll run
        #   - `PAID` - PAID
        #   - `DRAFT` - DRAFT
        #   - `APPROVED` - APPROVED
        #   - `FAILED` - FAILED
        #   - `CLOSED` - CLOSED
        @run_state = run_state
        # @type [RUN_TYPE_ENUM] The type of the payroll run
        #   - `REGULAR` - REGULAR
        #   - `OFF_CYCLE` - OFF_CYCLE
        #   - `CORRECTION` - CORRECTION
        #   - `TERMINATION` - TERMINATION
      ...
      def self.from_json(json_object:)
        ...
        run_state = Hris::RUN_STATE_ENUM.key(parsed_json["run_state"]) || parsed_json["run_state"]
        run_type = Hris::RUN_TYPE_ENUM.key(parsed_json["run_type"]) || parsed_json["run_type"]
      ...
      def to_json(*_args)
        {
          "id": @id,
          "remote_id": @remote_id,
          "run_state": Hris::RUN_STATE_ENUM[@run_state] || @run_state,
          "run_type": Hris::RUN_TYPE_ENUM[@run_type] || @run_type,
          "start_date": @start_date,
          "end_date": @end_date,
          "check_date": @check_date,
          "remote_was_deleted": @remote_was_deleted,
      ...
      def self.validate_raw(obj:)
        ...
        obj.run_state&.is_a?(Hris::RUN_STATE_ENUM) != false || raise("Passed value for field obj.run_state is not the expected type, validation failed.")
        obj.run_type&.is_a?(Hris::RUN_TYPE_ENUM) != false || raise("Passed value for field obj.run_type is not the expected type, validation failed.")
