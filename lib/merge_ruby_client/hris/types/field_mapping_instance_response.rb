...
module Merge
  module Hris
    class FieldMappingInstanceResponse
      ...
      # @param model [Merge::Hris::FieldMappingApiInstance]
      ...
      # @return [Merge::Hris::FieldMappingInstanceResponse]
      ...
      def self.from_json(json_object:)
        ...
        if parsed_json["model"].nil?
          ...
          model = Merge::Hris::FieldMappingApiInstance.from_json(json_object: model)
        end
        warnings = parsed_json["warnings"]&.map do |item|
          item = item.to_json
          Merge::Hris::WarningValidationProblem.from_json(json_object: item)
        end
        errors = parsed_json["errors"]&.map do |item|
          item = item.to_json
          Merge::Hris::ErrorValidationProblem.from_json(json_object: item)
        end
        logs = parsed_json["logs"]&.map do |item|
          item = item.to_json
          Merge::Hris::DebugModeLog.from_json(json_object: item)
        end
        new(
          model: model,
          warnings: warnings,
      ...
      def self.validate_raw(obj:)
        Merge::Hris::FieldMappingApiInstance.validate_raw(obj: obj.model)
