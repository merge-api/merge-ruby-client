...
module Merge
  module Hris
    ...
    class DataPassthroughRequest
      ...
      # @param data [String]
      # @param multipart_form_data [Array<Hris::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the `data` param if `request_format` is set to `MULTIPART`.
      ...
      # @return [Hris::DataPassthroughRequest]
      def initialize(method:, path:, base_url_override: nil, data: nil, multipart_form_data: nil, headers: nil,
        ...
        # @type [String]
        @data = data
        # @type [Array<Hris::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the `data` param if `request_format` is set to `MULTIPART`.
      ...
      def self.from_json(json_object:)
        ...
        method = Hris::METHOD_ENUM.key(parsed_json["method"]) || parsed_json["method"]
        path = struct.path
        base_url_override = struct.base_url_override
        data = struct.data
        multipart_form_data = parsed_json["multipart_form_data"]&.map do |v|
          v = v.to_json
          Hris::MultipartFormFieldRequest.from_json(json_object: v)
        end
        headers = struct.headers
        request_format = Hris::REQUEST_FORMAT_ENUM.key(parsed_json["request_format"]) || parsed_json["request_format"]
      ...
      def to_json(*_args)
        {
          "method": Hris::METHOD_ENUM[@method] || @method,
          "path": @path,
          "base_url_override": @base_url_override,
          "data": @data,
          "multipart_form_data": @multipart_form_data,
          "headers": @headers,
          "request_format": Hris::REQUEST_FORMAT_ENUM[@request_format] || @request_format,
          "normalize_response": @normalize_response
        }.to_json
      end
      ...
      def self.validate_raw(obj:)
        obj.method.is_a?(Hris::METHOD_ENUM) != false || raise("Passed value for field obj.method is not the expected type, validation failed.")
        ...
        obj.request_format&.is_a?(Hris::REQUEST_FORMAT_ENUM) != false || raise("Passed value for field obj.request_format is not the expected type, validation failed.")
