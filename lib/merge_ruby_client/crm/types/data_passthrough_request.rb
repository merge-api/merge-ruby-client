...
module Merge
  module Crm
    ...
    class DataPassthroughRequest
      ...
      attr_reader :data
      # @return [Array<Merge::Crm::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the
      ...
      attr_reader :headers
      # @return [Merge::Crm::RequestFormatEnum]
      ...
      # @param method_ [Merge::Crm::MethodEnum]
      ...
      #  matching the data's format
      # @param multipart_form_data [Array<Merge::Crm::MultipartFormFieldRequest>] Pass an array of `MultipartFormField` objects in here instead of using the
      #  `data` param if `request_format` is set to `MULTIPART`.
      # @param headers [Hash{String => Object}] The headers to use for the request (Merge will handle the account's
      #  authorization headers). `Content-Type` header is required for passthrough.
      #  Choose content type corresponding to expected format of receiving server.
      # @param request_format [Merge::Crm::RequestFormatEnum]
      ...
      # @return [Merge::Crm::DataPassthroughRequest]
      ...
      def self.from_json(json_object:)
        ...
        multipart_form_data = parsed_json["multipart_form_data"]&.map do |item|
          item = item.to_json
          Merge::Crm::MultipartFormFieldRequest.from_json(json_object: item)
        end
      ...
      def self.validate_raw(obj:)
        obj.method_.is_a?(Merge::Crm::MethodEnum) != false || raise("Passed value for field obj.method_ is not the expected type, validation failed.")
        ...
        obj.request_format&.is_a?(Merge::Crm::RequestFormatEnum) != false || raise("Passed value for field obj.request_format is not the expected type, validation failed.")
