  ...
  module Crm
    class IssuesClient
      ...
      def list(account_token: nil, cursor: nil, end_date: nil, end_user_organization_name: nil,
        ...
        response = @request_client.conn.get("/api/crm/v1/issues") do |req|
        ...
        Crm::PaginatedIssueList.from_json(json_object: response.body)
      end
      ...
      def retrieve(id:, request_options: nil)
        response = @request_client.conn.get("/api/crm/v1/issues/#{id}") do |req|
          req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
          req.headers["Authorization"] = request_options.api_key unless request_options&.api_key.nil?
          req.headers["X-Account-Token"] = request_options.account_token unless request_options&.account_token.nil?
          req.headers = { **req.headers, **(request_options&.additional_headers || {}) }.compact
        end
        Crm::Issue.from_json(json_object: response.body)
      end
    end
