...
module Merge
  module Ticketing
    class TicketingContactResponse
      # @return [Merge::Ticketing::Contact]
      attr_reader :model
      ...
      # @param model [Merge::Ticketing::Contact]
      ...
      # Deserialize a JSON object to an instance of TicketingContactResponse
      #
      ...
      def self.from_json(json_object:)
        ...
        if parsed_json["model"].nil?
          ...
          model = Merge::Ticketing::Contact.from_json(json_object: model)
        end
        warnings = parsed_json["warnings"]&.map do |item|
          item = item.to_json
          Merge::Ticketing::WarningValidationProblem.from_json(json_object: item)
        end
        errors = parsed_json["errors"]&.map do |item|
          item = item.to_json
          Merge::Ticketing::ErrorValidationProblem.from_json(json_object: item)
        end
        logs = parsed_json["logs"]&.map do |item|
          item = item.to_json
          Merge::Ticketing::DebugModeLog.from_json(json_object: item)
        end
        new(
          model: model,
          warnings: warnings,
      ...
      # Serialize an instance of TicketingContactResponse to a JSON object
      #
      # @return [String]
      def to_json(*_args)
        @_field_set&.to_json
      ...
      def self.validate_raw(obj:)
        Merge::Ticketing::Contact.validate_raw(obj: obj.model)
