  ...
  module Ats
    class AsyncPassthroughClient
      ...
      def create(request:, request_options: nil)
        response = @request_client.conn.post("/api/ats/v1/async-passthrough") do |req|
        ...
        Ats::AsyncPassthroughReciept.from_json(json_object: response.body)
      end
      ...
      def retrieve(async_passthrough_receipt_id:, request_options: nil)
        response = @request_client.conn.get("/api/ats/v1/async-passthrough/#{async_passthrough_receipt_id}") do |req|
          req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
          req.headers["Authorization"] = request_options.api_key unless request_options&.api_key.nil?
          req.headers["X-Account-Token"] = request_options.account_token unless request_options&.account_token.nil?
          req.headers = { **req.headers, **(request_options&.additional_headers || {}) }.compact
    ...
    class AsyncAsyncPassthroughClient
      ...
      # @return [Ats::AsyncAsyncPassthroughClient]
      def initialize(request_client:)
