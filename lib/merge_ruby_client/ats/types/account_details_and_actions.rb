  ...
  module Ats
    ...
    class AccountDetailsAndActions
      ...
      def self.from_json(json_object:)
        ...
        id = struct.id
        category = Ats::CATEGORY_ENUM.key(parsed_json["category"]) || parsed_json["category"]
        status = Ats::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM.key(parsed_json["status"]) || parsed_json["status"]
      ...
      def to_json(*_args)
        {
          "id": @id,
          "category": Ats::CATEGORY_ENUM[@category] || @category,
          "status": Ats::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM[@status] || @status,
          "status_detail": @status_detail,
          "end_user_origin_id": @end_user_origin_id,
          "end_user_organization_name": @end_user_organization_name,
          "end_user_email_address": @end_user_email_address,
      ...
      def self.validate_raw(obj:)
        ...
        obj.category&.is_a?(Ats::CATEGORY_ENUM) != false || raise("Passed value for field obj.category is not the expected type, validation failed.")
        obj.status.is_a?(Ats::ACCOUNT_DETAILS_AND_ACTIONS_STATUS_ENUM) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
