...
module Merge
  module Accounting
    class PaginatedExpenseList
      attr_reader :next_, :previous, :results, :additional_properties

      # @param next_ [String]
      # @param previous [String]
      # @param results [Array<Accounting::Expense>]
      ...
      def initialize(next_: nil, previous: nil, results: nil, additional_properties: nil)
        ...
        # @type [String]
        @previous = previous
        # @type [Array<Accounting::Expense>]
        @results = results
        # @type [OpenStruct] Additional properties unmapped to the current class definition
        @additional_properties = additional_properties
      end

      # Deserialize a JSON object to an instance of PaginatedExpenseList
      #
      ...
      def self.from_json(json_object:)
        ...
        results = struct.results.map do |v|
          v = v.to_h.to_json
          Accounting::Expense.from_json(json_object: v)
        end
        new(next_: next_, previous: previous, results: results, additional_properties: struct)
      end

      # Serialize an instance of PaginatedExpenseList to a JSON object
      #
      # @return [JSON]
      def to_json(*_args)
