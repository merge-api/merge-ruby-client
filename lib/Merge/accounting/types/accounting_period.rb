...
require "date"
require "date"
require_relative "accounting_period_status"
require "date"
require "date"
require "json"
...
Common models like `Invoice` and `Transaction` will have `AccountingPeriod` objects which will denote when they occurred.
    class AccountingPeriod
      ...
      # @param status [Accounting::AccountingPeriodStatus] 
      ...
      def initialize(start_date: nil, end_date: nil, status: nil, name: nil, id: nil, created_at: nil, modified_at: nil, additional_properties: nil)
        # @type [DateTime] Beginning date of the period
        @start_date = start_date
        # @type [DateTime] End date of the period
        @end_date = end_date
        # @type [Accounting::AccountingPeriodStatus] 
        @status = status
      ...
      def self.from_json(json_object:)
        ...
        status = Accounting::AccountingPeriodStatus.from_json(json_object: status)
        name = struct.name
        id = struct.id
        created_at = DateTime.parse(struct.created_at)
        modified_at = DateTime.parse(struct.modified_at)
        new(start_date: start_date, end_date: end_date, status: status, name: name, id: id, created_at: created_at, modified_at: modified_at, additional_properties: struct)
      end
      # Serialize an instance of AccountingPeriod to a JSON object
      #
      # @return [JSON]
      def to_json
        { "start_date": @start_date, "end_date": @end_date, "status": @status, "name": @name, "id": @id, "created_at": @created_at, "modified_at": @modified_at }.to_json()
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.status.nil?() || Accounting::AccountingPeriodStatus.validate_raw(obj: obj.status)
