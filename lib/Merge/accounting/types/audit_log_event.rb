...
module Merge
  module Accounting
    class AuditLogEvent
      ...
      # @param role [Accounting::AuditLogEventRole] Designates the role of the user (or SYSTEM/API if action not taken by a user) at the time of this Event occurring.  - `ADMIN` - ADMIN
      `DEVELOPER`
      `MEMBER`
      `API`
      `SYSTEM`
      ...
      # @param event_type [Accounting::AuditLogEventEventType] Designates the type of event that occurred.  - `CREATED_REMOTE_PRODUCTION_API_KEY` - CREATED_REMOTE_PRODUCTION_API_KEY
      ...
      # @return [Accounting::AuditLogEvent]
      def initialize(role:, ip_address:, event_type:, event_description:, id: nil, user_name: nil, user_email: nil,
        ...
        # @type [Accounting::AuditLogEventRole] Designates the role of the user (or SYSTEM/API if action not taken by a user) at the time of this Event occurring.
        `ADMIN`
        `DEVELOPER`
        `MEMBER`
        `API`
        ...
        # @type [Accounting::AuditLogEventEventType] Designates the type of event that occurred.
      ...
      def self.from_json(json_object:)
        ...
        role = Accounting::AuditLogEventRole.from_json(json_object: role)
        ip_address = struct.ip_address
        event_type = struct.event_type.to_h.to_json
        event_type = Accounting::AuditLogEventEventType.from_json(json_object: event_type)
      ...
      def self.validate_raw(obj:)
        ...
        Accounting::AuditLogEventRole.validate_raw(obj: obj.role)
        obj.ip_address.is_a?(String) != false || raise("Passed value for field obj.ip_address is not the expected type, validation failed.")
        Accounting::AuditLogEventEventType.validate_raw(obj: obj.event_type)
