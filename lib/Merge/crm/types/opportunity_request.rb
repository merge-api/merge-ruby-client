...
require_relative "opportunity_request_status"
require "date"
require "date"
...
TODO
    class OpportunityRequest
      attr_reader :name, :description, :amount, :owner, :account, :stage, :status, :last_activity_at, :close_date, :integration_params, :linked_account_params, :remote_fields, :additional_properties
      ...
      # @param status [Crm::OpportunityRequestStatus] The opportunity's status.  - `OPEN` - OPEN
- `WON` - WON
- `LOST` - LOST
      ...
      def initialize(name: nil, description: nil, amount: nil, owner: nil, account: nil, stage: nil, status: nil, last_activity_at: nil, close_date: nil, integration_params: nil, linked_account_params: nil, remote_fields: nil, additional_properties: nil)
        ...
        @stage = stage
        # @type [Crm::OpportunityRequestStatus] The opportunity's status. 
- `OPEN` - OPEN
- `WON` - WON
- `LOST` - LOST
        @status = status
      ...
      def self.from_json(json_object:)
        ...
        status = Crm::OpportunityRequestStatus.from_json(json_object: status)
  ...
  Crm::RemoteFieldRequest.from_json(json_object: v)
        ...
        new(name: name, description: description, amount: amount, owner: owner, account: account, stage: stage, status: status, last_activity_at: last_activity_at, close_date: close_date, integration_params: integration_params, linked_account_params: linked_account_params, remote_fields: remote_fields, additional_properties: struct)
      end
      # Serialize an instance of OpportunityRequest to a JSON object
      #
      # @return [JSON]
      def to_json
        { "name": @name, "description": @description, "amount": @amount, "owner": @owner, "account": @account, "stage": @stage, "status": @status, "last_activity_at": @last_activity_at, "close_date": @close_date, "integration_params": @integration_params, "linked_account_params": @linked_account_params, "remote_fields": @remote_fields }.to_json()
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.status.nil?() || Crm::OpportunityRequestStatus.validate_raw(obj: obj.status)
