...
module Merge
  module Hris
    class AuditLogEvent
      ...
      # @param role [Hris::AuditLogEventRole] Designates the role of the user (or SYSTEM/API if action not taken by a user) at the time of this Event occurring.  - `ADMIN` - ADMIN
      `DEVELOPER`
      `MEMBER`
      `API`
      `SYSTEM`
      ...
      # @param event_type [Hris::AuditLogEventEventType] Designates the type of event that occurred.  - `CREATED_REMOTE_PRODUCTION_API_KEY` - CREATED_REMOTE_PRODUCTION_API_KEY
      ...
      def initialize(role:, ip_address:, event_type:, event_description:, id: nil, user_name: nil, user_email: nil,
        ...
        # @type [Hris::AuditLogEventRole] Designates the role of the user (or SYSTEM/API if action not taken by a user) at the time of this Event occurring.
        `ADMIN`
        `DEVELOPER`
        `MEMBER`
        `API`
        ...
        # @type [Hris::AuditLogEventEventType] Designates the type of event that occurred.
      ...
      def self.from_json(json_object:)
        ...
        role = Hris::AuditLogEventRole.from_json(json_object: role)
        ip_address = struct.ip_address
        event_type = struct.event_type.to_h.to_json
        event_type = Hris::AuditLogEventEventType.from_json(json_object: event_type)
