...
Fetch from the `LIST TimeOffs` endpoint and filter by `ID` to show all time off requests.
    class TimeOffRequest
      attr_reader :employee, :approver, :status, :employee_note, :units, :amount, :request_type, :start_time, :end_time, :integration_params, :linked_account_params, :additional_properties
      ...
      # @param status [Hris::TimeOffRequestStatus] The status of this time off request.  - `REQUESTED` - REQUESTED
- `APPROVED` - APPROVED
- `DECLINED` - DECLINED
- `CANCELLED` - CANCELLED
- `DELETED` - DELETED
      # @param employee_note [String] The employee note for this time off request.
      # @param units [Hris::TimeOffRequestUnits] The measurement that the third-party integration uses to count time requested.  - `HOURS` - HOURS
- `DAYS` - DAYS
      # @param amount [Float] The time off quantity measured by the prescribed “units”.
      # @param request_type [Hris::TimeOffRequestRequestType] The type of time off request.  - `VACATION` - VACATION
- `SICK` - SICK
- `PERSONAL` - PERSONAL
- `JURY_DUTY` - JURY_DUTY
- `VOLUNTEER` - VOLUNTEER
- `BEREAVEMENT` - BEREAVEMENT
      ...
      def initialize(employee: nil, approver: nil, status: nil, employee_note: nil, units: nil, amount: nil, request_type: nil, start_time: nil, end_time: nil, integration_params: nil, linked_account_params: nil, additional_properties: nil)
...
- `BEREAVEMENT` - BEREAVEMENT
      ...
      def self.from_json(json_object:)
        ...
        approver = struct.approver.to_h.to_json()
        ...
        units = struct.units.to_h.to_json()
        units = Hris::TimeOffRequestUnits.from_json(json_object: units)
        amount = struct.amount
        request_type = struct.request_type.to_h.to_json()
        ...
        new(employee: employee, approver: approver, status: status, employee_note: employee_note, units: units, amount: amount, request_type: request_type, start_time: start_time, end_time: end_time, integration_params: integration_params, linked_account_params: linked_account_params, additional_properties: struct)
      end
      # Serialize an instance of TimeOffRequest to a JSON object
      #
      # @return [JSON]
      def to_json
        { "employee": @employee, "approver": @approver, "status": @status, "employee_note": @employee_note, "units": @units, "amount": @amount, "request_type": @request_type, "start_time": @start_time, "end_time": @end_time, "integration_params": @integration_params, "linked_account_params": @linked_account_params }.to_json()
      end
